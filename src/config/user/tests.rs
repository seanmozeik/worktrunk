...
#[test]
fn test_switch_snapshot_source_merges() {
    use crate::config::user::{Merge, SwitchConfig};
    let base: SwitchConfig = toml::from_str("snapshot-from = '/tmp/template'").unwrap();
    let override_config = SwitchConfig::default();
    assert_eq!(
        base.merge_with(&override_config).snapshot_from.as_deref(),
        Some(std::path::Path::new("/tmp/template"))
    );
}
